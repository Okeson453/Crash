...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM (VALUES
      ...
      ('ORDER_INTENT','FAILED'),
      ('ORDER_INTENT','RECONCILING'),
      ('ORDER_INTENT','SETTLED_WIN'),
      ('ORDER_INTENT','SETTLED_LOSS'),
      ('ORDER_INTENT','VOID'),
...
CREATE POLICY settlement_orders_tenant ON settlement_orders
  USING (current_setting('app.platform_role', true) = 'control_plane' OR tenant_id::text = current_setting('app.tenant_id', true))
  WITH CHECK (current_setting('app.platform_role', true) = 'control_plane' OR tenant_id::text = current_setting('app.tenant_id', true));
