...
export class StakeConfigurationService {
  ...
  async processStakeIncreasePayment(
    ...
    await getPool().query(
      `INSERT INTO payment_transactions (user_id, amount, amount_minor, currency, status, paystack_reference, channel)
       VALUES ($1, $2, $4, 'NGN', 'success', $3, 'stake_increase_fee')
       ON CONFLICT (paystack_reference) DO NOTHING`,
      [userId, fee, reference, Math.round(fee * 100)]
    );
    this.logger.info(
