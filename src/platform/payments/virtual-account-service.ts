...
export class VirtualAccountService {
  ...
  constructor(opts?: { paystack?: PaystackClient; notify?: NotifyFn }) {
    this.paystack = opts?.paystack ?? new PaystackClient(undefined, { optional: true });
  ...
  async handleTransferSuccess(payload: {
    ...
    const amountMinor = Math.trunc(payload.amount);
    ...
    if (amountMinor < expectedMinor) {
      ...
      await getPool().query(
        `INSERT INTO payment_transactions (
           user_id, virtual_account_id, paystack_reference, amount, amount_minor, currency, status,
           paid_at, channel, narration, metadata
         ) VALUES ($1,$2,$3,$4,$5,'NGN','failed',NOW(),$6,'underpayment',$7)
    ...
    const claim = await getPool().query(
      `INSERT INTO payment_transactions (
       ...
       ) VALUES ($1,$2,$3,$4,$5,'NGN','success',$6,$7,$8,$9,$10,$11)
  ...
  async createPendingTransaction(
    ...
    const result = await getPool().query(
      `INSERT INTO payment_transactions (user_id, amount, amount_minor, currency, status, paystack_reference)
       VALUES ($1, $2, $4, 'NGN', 'pending', $3)
       RETURNING id`,
      [userId, amount, reference, Math.round(amount * 100)]
    );
  ...
  async verifyPendingTransaction(
    ...
    const expectedMinor = Math.round(Number(plan.priceMonthly) * 100);
    if (Number(tx.amount_minor) < expectedMinor) throw new Error('Underpayment cannot be manually verified');
    const claim = await getPool().query(
      `UPDATE payment_transactions SET status = 'success', paid_at = NOW()
       WHERE id = $1 AND status <> 'success' RETURNING id`,
      [txId],
    );
