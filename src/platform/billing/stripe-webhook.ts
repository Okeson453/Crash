...
export async function handleStripeEvent(event: {
  ...
  switch (event.type) {
    ...
    case 'invoice.paid': {
      ...
      const periodEnd = Number(obj.period_end ?? obj.current_period_end ?? 0);
      if (subscriptionId) {
        await getPool().query(
          `UPDATE subscriptions SET status = 'active',
             current_period_end = CASE WHEN $2 > 0 THEN to_timestamp($2) ELSE current_period_end END,
             updated_at = NOW() WHERE payment_provider_subscription_id = $1`,
...
export async function processStripeWebhookHttp(params: {
  ...
  if (params.enqueueOnly) {
    ...
    const queued = await new WebhookInbox().enqueue({ provider: 'stripe', eventId, signature: params.signatureHeader, rawBody: params.rawBody });
    return { status: 200, body: { received: true, queued: queued.inserted, eventId } };
  }
