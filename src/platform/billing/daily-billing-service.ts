...
export class DailyBillingService {
  ...
  async activateDailySubscription(params: {
    ...
    const claim = await getPool().query(
      `INSERT INTO daily_subscriptions (user_id, plan_id, status, paid_date, paid_at, expires_at, payment_reference)
       VALUES ($1, $2, 'active', CURRENT_DATE, NOW(), $3, $4)
       ON CONFLICT (user_id, paid_date) DO NOTHING RETURNING id`,
      [params.userId, params.planId, tomorrow, params.reference],
    );
    if (!claim.rowCount) {
      const existing = await this.getTodaySubscription(params.userId);
      return { success: true, message: 'Daily access already activated.', expiresAt: existing?.expiresAt ?? tomorrow };
    }
    ...
    if ((!instance || instance.status !== 'running') && plan) {
      ...
      })).catch((error) => this.logger.error({ component: 'DailyBilling', userId: params.userId, error: String(error) }, 'Async tenant provisioning failed'));
    }
  ...
  > {
    ...
    for (const row of result.rows) {
      ...
      try {
        ...
        this.logger.error({ component: 'DailyBilling', userId: String(row.user_id), error: String(error) }, 'Failed to pause expired tenant');
      }
      expired.push({
