...
export async function startControlPlane(): Promise<ControlPlaneHandles> {
  ...
  const stopWebhookProcessor = await webhookInbox.startProcessor(async (item) => {
    if (item.provider === 'stripe') {
      const result = await processStripeWebhookHttp({ rawBody: item.rawBody });
      if (result.status >= 400) throw new Error(`Stripe webhook failed: ${result.status}`);
    } else {
      const result = await processPaystackWebhookHttp({ rawBody: item.rawBody, enqueueOnly: false });
      if (result.status >= 500) throw new Error(`Paystack webhook failed: ${result.status}`);
    }
  });
...
async function handleHttp(
  ...
  try {
    ...
    if (method === 'POST' && url === '/webhooks/stripe') {
      ...
      const result = await processStripeWebhookHttp({ rawBody, signatureHeader: signature, enqueueOnly: true });
    ...
    if (method === 'POST' && (url === '/webhooks/paystack' || url === '/paystack')) {
      ...
      const result = await processPaystackWebhookHttp({ rawBody, signatureHeader: signature, enqueueOnly: true });
      res.writeHead(result.status, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify(result.body));
      return;
    }
