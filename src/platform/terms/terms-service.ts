...
import { GUIDELINES_VERSION } from './guidelines.js';
...
export class TermsAndConditionsService {
  ...
  async hasUserAcceptedGuidelines(
    userId: string,
  ...
  async acceptGuidelines(params: {
    userId: string;
    version?: string;
    ipAddress?: string;
    userAgent?: string;
  }): Promise<void> {
    const version = params.version ?? GUIDELINES_VERSION;
    await getPool().query(
      `INSERT INTO terms_and_conditions (version, title, content, effective_date, is_active)
       SELECT $1, $2, $3, NOW(), false
       WHERE NOT EXISTS (SELECT 1 FROM terms_and_conditions WHERE version = $1)`,
      [version, 'CrashWave Usage Guidelines', 'See in-app guidelines message.']
    );
