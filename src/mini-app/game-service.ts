...
export class MiniGameService {
  ...
  async placeBet(userId:string, amount:number, autoCashout:number|null, idempotencyKey:string):Promise<MiniBet> {
    ...
    try { await client.query('BEGIN'); const existing=await client.query('SELECT * FROM mini_app_bets WHERE idempotency_key=$1 FOR UPDATE',[idempotencyKey]); if(existing.rows[0]){await client.query('COMMIT');return this.rowToBet(existing.rows[0]);}
      ...
      const result=await client.query('INSERT INTO mini_app_bets(user_id,round_id,amount,auto_cashout,state,idempotency_key) VALUES($1,$2,$3,$4,\'placed\',$5) RETURNING *',[userId,this.roundId,amount,autoCashout,idempotencyKey]); await client.query('COMMIT'); const bet=this.rowToBet(result.rows[0]); this.emit('bet:placed',{bet,userId,serverTime:new Date().toISOString()}); return bet;
  ...
  private async settleLosses():Promise<void>{if(!this.roundId)return;await getPool().query('UPDATE mini_app_bets SET state=\'lost\',pnl=-amount,settled_at=NOW() WHERE round_id=$1 AND state IN (\'placed\',\'active\')',[this.roundId]);}
