...
import { normalizeProxyPool } from '../network/proxy-manager';
...
export function loadAndValidateConfig(configPath?: string): AppConfig {
  const raw = loadConfig(configPath) as Record<string, unknown>;
  // Env may pass APP_PROXY__POOL as JSON string or newline/comma list
  if (raw.proxy && typeof raw.proxy === 'object' && raw.proxy !== null) {
    const px = raw.proxy as Record<string, unknown>;
    if (px.pool !== undefined) {
      px.pool = normalizeProxyPool(px.pool);
    }
  }
