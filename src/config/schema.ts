...
export const TelegramConfigSchema = z.object({
  /** Platform admin Telegram IDs only — not a tenant access gate */
...
export const ProxyConfigSchema = z.object({
  ...
  /** Primary proxy server, e.g. http://host:port */
  server: z.string().optional(),
  username: z.string().optional(),
  password: z.string().optional(),
  /**
   * Optional pool of proxies (Webshare-style host:port:user:pass or http://user:pass@host:port).
   * When set, ProxyManager picks the first healthy entry (or rotates per rotationMode).
   * APP_PROXY__POOL can be JSON array or newline/comma-separated string.
   */
  pool: z.array(z.string()).optional(),
  sticky: z.boolean().default(true),
  rotationMode: z.enum(['never', 'on-profile-recreate', 'daily', 'round-robin']).default('never'),
  provider: z.enum(['generic', 'brightdata', 'oxylabs', 'iproyal', 'webshare', 'custom']).default('generic'),
}).refine((data) => !data.enabled || !!data.server || (data.pool && data.pool.length > 0), {
  message: 'proxy.server or proxy.pool is required when proxy.enabled = true',
});
