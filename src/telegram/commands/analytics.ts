...
import { escapeMarkdown } from '../formatters/templates';
...
async function handleSummary(deps: RouterDependencies): Promise<CommandResult> {
  ...
  const message = [
    ...
    `*Hit Rate:* ${hitRate !== null ? escapeMarkdown(`${(hitRate * 100).toFixed(1)}%`) : 'N/A'}`,
...
async function handleToday(deps: RouterDependencies): Promise<CommandResult> {
  ...
  const message = [
    `📅 *Today — ${escapeMarkdown(dailyKey)}*`,
    '',
    `*Entries:* ${entriesConfirmed}`,
    `*Wins/Losses:* ${wins}W / ${losses}L`,
    `*Net P&L:* ${pnlEmoji} ${formatCurrency(netPnl)}`,
    balanceChange !== null ? `*Balance Change:* ${formatCurrency(balanceChange)}` : '',
    `*Cash-out Rate:* ${cashOutSuccessRate !== null ? escapeMarkdown(`${(cashOutSuccessRate * 100).toFixed(1)}%`) : 'N/A'}`,
  ].filter(Boolean).join('\n');
...
async function handleWindow(
  ...
  const lines = [
    ...
    hitRate !== undefined ? `*Hit Rate:* ${escapeMarkdown(`${(hitRate * 100).toFixed(2)}%`)}` : null,
    `*Net P&L:* ${formatCurrency(netPnl)}`,
    maxDd !== undefined ? `*Max Drawdown:* ${escapeMarkdown(`${(maxDd * 100).toFixed(2)}%`)}` : null,
    cashOutRate !== undefined ? `*Cash-out Success:* ${escapeMarkdown(`${(cashOutRate * 100).toFixed(1)}%`)}` : null,
    avgLatency !== undefined ? `*Avg Tick Latency:* ${escapeMarkdown(`${avgLatency.toFixed(0)}ms`)}` : null,
    '',
...
function formatCurrency(value: number): string {
  const sign = value >= 0 ? '+' : '';
  return escapeMarkdown(`${sign}${value.toFixed(2)}`);
}
