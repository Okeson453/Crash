...
export function formatWin(params: {
  ...
  const lines = [
    '🏆 *WIN*',
    '',
    `*Bet ID:* \`${escapeMarkdown(betId)}\``,
    `*Round:* \`${escapeMarkdown(roundId)}\``,
    `*Stake:* ${escapeMarkdown(stake.toFixed(2))}`,
    `*Cash Out:* ${escapeMarkdown(cashOutMultiplier.toFixed(2))}x`,
    `*P&L:* 🟢 \\+${escapeMarkdown(pnl.toFixed(2))}`,
  ];
...
export function formatLoss(params: {
  ...
  const lines = [
    ...
    `*Round:* \`${escapeMarkdown(roundId)}\``,
    `*Stake:* ${escapeMarkdown(stake.toFixed(2))}`,
    `*Crashed At:* ${escapeMarkdown(crashPoint.toFixed(2))}x`,
    `*P&L:* 🔴 ${escapeMarkdown(pnl.toFixed(2))}`,
  ];
...
export function formatDailySummary(params: {
  ...
  const lines = [
    `📋 *Daily Report — ${escapeMarkdown(dailyKey)}*`,
    '',
    `*Entries:* ${entriesConfirmed}`,
    `*Wins/Losses:* ${wins}W / ${losses}L`,
    `*Hit Rate:* ${hitRate !== null ? escapeMarkdown(`${(hitRate * 100).toFixed(1)}%`) : 'N/A'}`,
    `*Net P&L:* ${pnlEmoji} ${escapeMarkdown(formatCurrency(netPnl))}`,
    `*Max Drawdown:* ${escapeMarkdown(formatPercentage(maxDrawdown))}`,
  ];
