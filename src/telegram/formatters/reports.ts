...
import { FormattedMessage, formatDailySummary, escapeMarkdown } from './templates';
...
export function formatLearningCurve(data: LearningCurveData): FormattedMessage {
  ...
  const lines = [
    ...
    `*Cumulative P&L:* ${pnlEmoji} ${escapeMarkdown(formatCurrency(cumulativePnl))}`,
    `*Latest Win Rate:* ${winRateTrendEmoji} ${latestWinRate !== null ? escapeMarkdown(`${(latestWinRate * 100).toFixed(1)}%`) : 'N/A'}`,
    '',
  ];
