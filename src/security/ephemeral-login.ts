...
export interface EphemeralLoginPayload {
  ...
  chatId: number;
  createdAt: number;
  /** Optional tenant user id (multi-tenant control plane) */
  tenantUserId?: string;
}

const DEFAULT_TTL_MS = 90_000;

/** In-process map: chatId → pending login */
const pendingByChat = new Map<number, EphemeralLoginPayload>();

/** One-shot slots for tenant engines (tenantUserId → payload) */
const pendingByTenant = new Map<string, EphemeralLoginPayload>();

function zeroPassword(p: EphemeralLoginPayload | undefined): void {
  if (!p) return;
  // Overwrite string content as best-effort (JS strings are immutable; drop refs)
