...
export function reportWebVitals(): void {
  ...
  try { new PerformanceObserver((list) => { const entries=list.getEntries(); const last=entries[entries.length-1]; if(last) logger.info('Interaction observed',{value:last.duration}); }).observe({type:'event',buffered:true} as PerformanceObserverInit); } catch { /* unsupported */ }
